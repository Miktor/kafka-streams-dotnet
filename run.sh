...
docker-compose -f docker-compose.yml up -d schema-registry akhq
docker exec -i ${kafkaContainerId} kafka-topics --bootstrap-server broker:29092 --topic test_input_table --create --partitions 4 --replication-factor 1 > /dev/null 2>&1
docker exec -i ${kafkaContainerId} kafka-topics --bootstrap-server broker:29092 --topic test_output_topic --create --partitions 4 --replication-factor 1 > /dev/null 2>&1
echo "Topics created"
...
if [ $# -gt 0 ]
  then
     dotnet run -f net6.0 --project Streams/Streams.csproj --no-build --no-restore
fi